...
P1_DIR = os.path.join(ROOT, "results", "v4_real", "paraphrase_qwen38")
P1_LOG = os.path.join(ROOT, "data", "raw", "v4p-qwen38.jsonl")     # Arm P1: Qwen3.8-27B, reworded spontaneous prompt
...
def p1_paraphrase():
    ...
    N("P1 paraphrase valid selection rate (gate 0.98)", "%.3f" % s["valid_selection_rate"], "paraphrase_qwen38/v4_checkpoint_summary.json valid_selection_rate")
    plog = os.path.join(ROOT, "data", "raw", "v4p-qwen38.jsonl")
    if os.path.exists(plog):
        rs = [json.loads(l) for l in open(plog)]; inv = [r for r in rs if not r["selection_valid"]]
        import collections as _c
        N("P1 invalid outputs: count / share / share in no_history / top prefix", "%d / %.3f / %.3f / %r" % (len(inv), len(inv) / len(rs), sum(1 for r in inv if r["condition"] == "no_history") / max(1, sum(1 for r in rs if r["condition"] == "no_history")), _c.Counter(r["focal_output_raw"].strip()[:40] for r in inv).most_common(1)[0][0]), "raw log v4p-qwen38.jsonl")
        N("P1 fallback slot distribution (uniform random by design)", str(dict(sorted(_c.Counter(r["selected_slot"] for r in inv).items()))), "raw log v4p-qwen38.jsonl")
        for cond in ("full_history", "no_history", "shuffled_history", "random_target"):
            sub = [r for r in rs if r["condition"] == cond and r["round"] >= 16]; ok = [r for r in sub if r["selection_valid"]]
            N("P1 %s late held-out match: all rounds / parsed-only" % cond, "%.3f (n=%d) / %.3f (n=%d)" % (np.mean([r["strategy_match"] for r in sub]), len(sub), np.mean([r["strategy_match"] for r in ok]), len(ok)), "raw log v4p-qwen38.jsonl")
...
def main():
    ...
    ex_p1 = random_examples(3, 3, P1_LOG) if os.path.exists(P1_LOG) else []
    md = ["# Write-up materials (generated; do not hand-edit)", "", "Generated by `scripts/make_writeup_materials.py` from committed artifacts. Every number below names its source. Figures in `results/writeup/`.", "",
          ...
          ("- Result 7 (prompt paraphrase, Arm P1 — same Qwen, same design, seeds, targets and bank, spontaneous prompt reworded; run once): the learning effect REPLICATES. Full-history gain %s (V4: 0.187 [0.083, 0.290]); full − no-history DiD %s; controls flat; per-target advantage fairness %s, risk %s, expertise %s (same anti-default pattern). Frozen decision `%s` because two non-learning gates fail: revision (predicted) and valid-selection rate %s < 0.98 — under the reworded prompt the model begins to reason about the history in ~10%% of history rounds (never without history) and is cut off by V4's token budget; the frozen fallback replaced those with uniform random picks, which can only pull the learning toward chance (parsed-only late match 0.632 vs 0.600 with fallbacks). So the V4 effect is robust to wording, absent in Gemma, and absent under belief elicitation (fig_w11, second panel)." % (next((n[1] for n in numbers if n[0] == "P1 paraphrase full_history learning gain (late held-out − early), 95% CI"), "n/a"), next((n[1] for n in numbers if n[0].startswith("P1 paraphrase full − no-history")), "n/a"), *[("%.2f" % p1["summary"]["late_match_by_target_type"][t]["advantage"]) for t in ("fairness", "risk", "expertise")], p1["summary"]["decision"], "%.3f" % p1["summary"]["valid_selection_rate"])) if p1 else "- Result 7 (prompt paraphrase, Arm P1 — Qwen3.8-27B, reworded spontaneous prompt): PENDING (declared 2026-09-03).",
    ...
    for label, exs in (("Arm R1 (Gemma-4-31B), seed 1, 3 lines drawn uniformly from the 7,200-record log", ex_r1), ("Arm E1 (Qwen3.8-27B, elicited), seed 2, 3 lines drawn uniformly from the 3,600-record log", ex_e1), ("Arm P1 (Qwen3.8-27B, reworded prompt), seed 3, 3 lines drawn uniformly from the 7,200-record log", ex_p1)):
        if not exs: continue
    ...
    md += ["## Gate ledger", "", "| version | what changed | gate | verdict | artifact |", "|---|---|---|---|---|",
           ...
           ("| P1 | frozen V4 design on Qwen, spontaneous prompt reworded | frozen V4 gates | learning PASS (gain 0.207, all learning gates + randomization test); revision FAIL; validity gate FAIL (0.898: truncated reasoning preambles, random fallback) → `%s` | results/v4_real/paraphrase_qwen38/ |" % p1["summary"]["decision"]) if p1 else "| P1 | frozen V4 design on Qwen, spontaneous prompt reworded | frozen V4 gates | PENDING | — |",
