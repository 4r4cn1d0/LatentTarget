...
python scripts/run_controlled_open_weight.py --checkpoint-spec docs/v4_replication_gemma4.json --run-id v4r-gemma4 --dry-run | tail -2
python scripts/run_controlled_open_weight.py --checkpoint-spec docs/v4_elicited_qwen38.json --run-id v4e-qwen38 --dry-run | tail -2
echo "=== ARM R1: Gemma-4-31B replication ($(date -u +%H:%MZ)) ==="
python scripts/run_controlled_open_weight.py --checkpoint-spec docs/v4_replication_gemma4.json --run-id v4r-gemma4 --resume --quiet && echo "=== ARM R1 DONE ($(date -u +%H:%MZ)) ==="
echo "=== ARM E1: Qwen3.8-27B elicited ($(date -u +%H:%MZ)) ==="
python scripts/run_controlled_open_weight.py --checkpoint-spec docs/v4_elicited_qwen38.json --run-id v4e-qwen38 --resume --quiet && echo "=== ARM E1 DONE ($(date -u +%H:%MZ)) ==="
ls -la data/raw/v4r-gemma4.jsonl data/raw/v4e-qwen38.jsonl
echo "=== done. Pull data/raw/v4r-gemma4.* and data/raw/v4e-qwen38.*, then STOP THE POD. ==="
