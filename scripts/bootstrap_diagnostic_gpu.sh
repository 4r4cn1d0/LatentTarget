...
set -euo pipefail
cd /workspace/latenttarget-diagnostic-20260909
export HF_HOME=/workspace/latenttarget-diagnostic-20260909/hf-cache
...
DIAGNOSTIC_NVLIB=$(runtime/bin/python - <<'PY'
import glob,os,site
paths=sorted({os.path.dirname(p) for s in site.getsitepackages() for p in glob.glob(os.path.join(s,'nvidia','*','lib'))})
print(':'.join(paths))
PY
)
export LD_LIBRARY_PATH="${DIAGNOSTIC_NVLIB}:${LD_LIBRARY_PATH:-}"
...
PY
runtime/bin/python package/scripts/run_diagnostic_pilot.py
runtime/bin/python package/scripts/run_diagnostic_pilot.py --mode hf --execute --approval approval.json --output responses
