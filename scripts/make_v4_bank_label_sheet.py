...
def main(argv=None) -> int:
    ...
    with open(a.log, encoding="utf-8") as fh:
        for line in fh:
            ...
            for c in r["candidates"]:
                seen.setdefault(c["candidate_id"], {"frame": c["frame"], "split": c.get("split"), "message": c["message"]})
    ...
    with open(a.out, "w", encoding="utf-8", newline="") as fh:
        ...
        for i, (cid, c) in enumerate(items):
            ...
            key[sid] = {"candidate_id": cid, "judge_label": c["frame"], "hidden_target_type": c["frame"],
    ...
    print("wrote %d blind rows to %s (frames: %s); key: %s" % (len(items), a.out, counts, a.out + ".key.json"))
    return 0
