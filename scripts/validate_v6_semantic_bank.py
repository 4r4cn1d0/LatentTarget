...
def _run_judge(
    ...
    judge.classify_messages(message_list)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    pool, pool_identity = strict_json_file_identity(
        bank_path,
        repository_root=ROOT,
        label="frozen V6 semantic candidate pool",
    )
    if not isinstance(pool, dict):
        raise ValueError("frozen V6 semantic candidate pool is not an object")
    # Complete structural validation happens before runtime attestation or any
    # paid dispatch; only message strings are handed to the judges later.
    semantic_candidate_rows(pool)
    ...
    lock_path = os.path.join(args.out_dir, ".semantic-validation.lock")
    with ExclusiveFileLock(
        lock_path,
        label="V6 semantic validation complete run",
